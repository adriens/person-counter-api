...
public class PersonCounterServiceTests {
    ...
    public void detectTest() throws IOException, ModelException, TranslateException {
        ...
        // TESTS ON ATM IMAGE
        Image img = ImageFactory.getInstance().fromInputStream(getClass().getClassLoader().getResourceAsStream("dab1.jpg"));
        ...
        assertEquals(7, numberOfDetectedPersonsAbove80Percent, "Wrong number of persons detected above 80% probability on the first image");
        assertEquals(4, numberOfDetectedPersonsAbove90Percent, "Wrong number of persons detected above 90% probability on the first image");
    ...
    public void getMetaDatasTest() throws IOException, SAXException, TikaException {
        HashMap<String, String> map = service.getMetaDatas("dab1.jpg");
        assert map.size() == 100;
